...
export const GameCanvas: React.FC<TGameCanvasProps> = ({ onHeroClick, heroSettings }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [mousePos, setMousePos] = useState<{ x: number, y: number }>({ x: 0, y: 0 });
  const [scores, setScores] = useState<{ [key: number]: number }>({ 1: 0, 2: 0 });
  const heroes = useRef<THero[]>([
    { id: 1, x: 50, y: 300, radius: 20, color: heroSettings[1].color, speed: heroSettings[1].speed, direction: 1 },
    { id: 2, x: 750, y: 300, radius: 20, color: heroSettings[2].color, speed: heroSettings[2].speed, direction: -1 }
  ]);
  ...
  useEffect(() => {
    const intervalIds = heroes.current.map(hero => {
      ...
      hero.speed = settings.speed;
      return setInterval(() => shootSpell(hero), 1000 / settings.frequency);
    });
  ...
  const updateSpells = () => {
    ...
    spells.current = spells.current.filter(spell => spell.x > 0 && spell.x < 800);
  };
  ...
  const handleMouseCollisions = () => {
    heroes.current.forEach(hero => {
      const dx = hero.x - mousePos.x;
      const dy = hero.y - mousePos.y;
      const distance = Math.sqrt(dx * dx + dy * dy);
      if (distance < hero.radius + 10) { // 10 - радиус курсора мыши
        hero.direction *= -1;
      }
    });
  };
  ...
  return (
    <>
      <canvas ref={canvasRef} width={800} height={600} onClick={handleCanvasClick} onMouseMove={handleMouseMove} />
